...
2. **git 分支隔离**：各开 feature 分支，避免提交冲突
3. **共享 key 限流**：多个并发共享同一 DeepSeek key，注意 API 限流（429 时重试/排队）
4. **编排与审查**：编排岗（如 Athena）拆模块边界，审查岗（如 Eos）独立验收
