...
**飞书（Feishu）接入要点**：插件在 `plugins/platforms/feishu/`（官方自带），`.env` 配 `FEISHU_APP_ID` + `FEISHU_APP_SECRET` + `FEISHU_ALLOW_ALL_USERS=true` + **`FEISHU_GROUP_POLICY=open`**，config.yaml 加 `platforms.feishu.enabled: true` 即可。**无独立 toolset**（`hermes-feishu` 只是线程名前缀，不要加进 platform_toolsets 否则启动报错）。飞书支持多 agent 协作（Approach B：每档案一个飞书应用 + profile，群 @ 触发），比 QQ 强——AIGC 机器人也能进普通飞书群。**完整接入步骤、7 项权限清单（99991672 Access denied 排查）、完整 19 项群聊权限清单、API 实测验证权限（`receive_id_type` 必须 query 参数）、「群里 @ 没反应」的双重坑诊断路径（require_mention + group_policy）、Monaco 编辑器自动化坑全部见 `references/feishu-setup.md`**（v4，2026-08-09 实战），批量导入 JSON 模板在 `templates/feishu-scopes-batch-import.json`。⚠️ `im:message.reaction` 在飞书平台**不存在**（实测批量导入报错），表情回应由 `im:message` 涵盖——不要把它加进权限清单。**⚠️ 群消息收不到的 TWO traps**（默认值都会静默拒绝，只打 debug 日志）：① `require_mention` 默认 True → 群聊自主接话必须显式配 `extra.require_mention: false`；② **`group_policy` 默认 `allowlist` 且 `FEISHU_ALLOWED_USERS` 为空 → 所有群消息被 `group_policy_rejected` 静默丢弃**——`FEISHU_ALLOW_ALL_USERS=true` 只放行私聊不管群聊，必须 `.env` 加 `FEISHU_GROUP_POLICY=open` 才能真正收到群消息（2026-08-09 实测：这是「私聊通、群聊死」的最终根因）。**★ require_mention 三态**：`true`（只回 @）/ `false`（全员抢答，@ 一个全员回）/ `"smart"`（无 @ 自主接话 + 有 @ 只回被 @ 的，@全体全员回）——smart 是本地 adapter 补丁（`_normalize_require_mention` + `_admit` smart 分支，4 处改动，升级会被覆盖需重打），完整补丁代码见 `references/feishu-setup.md` §3a。**⚠️ 双运行时坑（smart 补丁"没生效"的真凶）**：Desktop gateway 用独立运行时 `~/.hermes-web-ui/desktop-runtime/hermes/<version>/win-x64/python/Lib/site-packages/`，**不加载 `$HERMES_HOME/hermes-agent/` 的代码**——adapter 补丁必须同时打到两份（验证：grep 运行时副本的 `require_mention == "smart"`），否则 Desktop 仍跑旧代码、`"smart"` 退化成 `false`（全员抢答）。详见 `references/feishu-setup.md` §3a 双运行时节。⚠️ `im:message.reaction` 在飞书平台**不存在**（实测批量导入报错），表情回应由 `im:message` 涵盖——不要把它加进权限清单。
