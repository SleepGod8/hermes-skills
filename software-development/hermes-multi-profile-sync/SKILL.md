...
- **新模块插在固定小节标题前最稳**：如 `## 茶会日常+野兽+疯狂口穴 🆕`、`## 共通色情机制 🆕`、`## 🎂 女仆年龄排序` 在每个档案里出现且唯一，是稳定锚点；插入时保留原有 `---` 分隔结构。
- **新增成员进家族时，default 的跨档案联动要整套更新**：联动标题成员列表（如 `Hermes×Iris ↔ Artemis/Athena/Hebe/Nemesis`）、茶会人数与台词、大乱斗、养成学院、修罗场反应、野兽组合行，并补年龄排序模块——default SOUL.md + lewd-maid 镜像两处都改。
- **配对互动要双边写入**：女仆×女仆配对（如 Artemis × Athena「傲娇对冰山」）写进**双方档案**各一份——A 档案的配对模块加 `×B`（A 视角），B 档案若没有配对模块则新建 `## 与其他女仆的配对互动 🆕` 加 `×A`（B 视角，台词不同）。组合名约定：每对末尾一行 `- 组合名：xxx`。
- **演示文本可复用写入**：用户要求"演示一下"某个设定、满意后说"写进档案"时，把演示场景微调成"场景细节"模块原样写入（如"妹妹们围攻求展示"场景细节写进 Athena 清醒野兽模块），比重新设计更快且与用户所见一致。

## 用户偏好（配置角色色情设定时）
- 流程：给建议清单（分共通/角色专属，带清晰选项）→ `clarify` 选择 → 用户可能要求先"演示"一段效果 → 确认后才批量写入 → 写完后提醒"新开会话（/new）才生效"。
- **演示演出要点**：贴合该档案已有设定（人设、口头禅、专属机制）演出，别凭空捏造人设；演示后问是否写入。
- **精确指定**：用户会从清单只挑编号（"添加4"只写第4个）、指定修改细节（"Athena不要添加s属性"、"报应惩罚play以nemesis自己高潮后屈服求饶结束"）——严格按指定范围执行，不自由发挥。
