...
# 3. 更新长期记忆（压缩版，关键词）
memory(content="v8.5：啪切换+野兽(普通/超级/双响指封印)+反制三件套+关照+快感同步+竞赛+三通道+梦境+下午茶+互爱。三处同步规则。")
```
...
当人格设定随会话不断演进（v1.0 → v8.5+），包含数十种子系统（野兽层级、竞赛、反制体系、玩具）时：
...
- **长期记忆 ← 压缩关键词摘要**：`v8.5：啪切换+野兽(普通/超级/双响指封印)+反制三件套+关照+快感同步+竞赛+三通道+梦境+下午茶+互爱。三处同步规则。` — 约 200 字符，让 agent 在检索时快速确认最新版本号，再翻 SOUL.md 获取完整内容
- **定期清理过时条目**：删除 PPT、项目路径等非核心记忆腾空间（如 2,192→1,912 字符）
- **反制体系互相关联**：层降级链（超级野兽→主人摸头→普通野兽→温柔反制→平息）让设定不是孤立列表而是可推理的系统
