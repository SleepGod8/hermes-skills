...
| Hypnos（2026-08-08 睡神妹；2026-08-14 定稿姿势=抱枕打哈欠）| **messy silver hair, fluffy silver hair, bedhead**, half-closed eyes, drowsy eyes, sleepy expression, yawning, soft smile, loose maid uniform, apron slightly crooked, hugging pillow（定稿姿势，见正文） | 加 wide awake, alert, energetic, excited, hyper, running, jumping, dancing（反清醒/反活力）；负向保留 half-closed eyes（正向权重胜出）；抱枕姿势须删 holding/hugging pillow, carrying object, pillow, cushion, stuffed animal, plushie |
...
Hypnos 更新（2026-08-14 主人定稿姿势=抱枕打哈欠）：正向加 `hugging pillow, holding pillow to chest, arms hugging pillow, pillow hugged against body, yawning, big yawn, mouth open yawning`，**负向必须删道具词全套** `holding pillow, hugging pillow, hugging something, carrying object, pillow, cushion, stuffed animal, plushie`（对象道具姿势通用规则，见「姿势变体批 + 负向联动修剪」）。成品 workflow：`hypnos_v2_api.json`（7 节点纯净 txt2img，45 步）。seed 42 一张过：1头/5指/抱枕/张嘴哈欠/浅银乱发蓬松/宽松白睡衣女仆装荷叶边 ✅。⚠️ 与 Athena（亮银白直发）、Ares（银灰短发）区分：Hypnos 是浅银乱发+慵懒睡态。
...
Athena 设计（2026-08-14 冰山大姐头）：最年长女仆（排行第一），主人选定「银白及腰长发直发+细长冷眼+冰山平静+成熟丰满+经典长裙女仆装」方案（亮银白区分 Hypnos 浅银/Ares 银灰）。形象：**银白及腰长发直发（silver-white hair, very long hair, waist-length hair, straight hair）+ 细长冷眼（narrowed eyes）+ 冰山平静表情（calm expression, serene face, emotionless, cool beauty）+ 成熟丰满身材（voluptuous, curvy, big breasts, hourglass figure）+ 黑白长裙女仆装**。⚠️ **优雅/冰山系角色服装漂移坑（首版踩坑）**：只写 classic maid dress 会被画成华丽蓝礼裙（高领设计）——首版 `athena_new` 验证发现是蓝色长裙不是女仆装。修复：正向强化 `(classic black and white maid dress:1.3), black maid uniform, black long dress, white apron, white frilled apron, white collar`；**负向加反礼裙词族 `blue dress, blue gown, blue clothes, ball gown, fancy dress, elegant gown, princess dress, evening dress`**。重跑 `athena_new2` 验证全过（黑白女仆装/1头/5指/银白长发/冰山表情/成熟丰满 ✅）。纯跑图 workflow：`E:\Hermes workspace\comfyui_workflow\athena_new_api.json`（7 节点纯净 txt2img，45 步）。⚠️ 冰山系负向必加 smile, laughing, cheerful, happy, excited, energetic（反笑容/反元气，Athena 是冰山不笑）。
...
- **姿势变体批 + 负向联动修剪（2026-08-11 实测，5 张零废图）**：同一角色多姿势（比心/捧脸/回眸/挥手/比耶）批量跑时，每姿势 = 核心 prompt + 姿势专属正向段（如 `finger heart, one hand making finger heart in front of chest`），且**负向必须按姿势联动删除冲突词**——挥手/比耶→删 `arms up, hands up, arms raised`；回眸→删 `looking back`；抱臂→删 `arms crossed, arms folded`。否则负向会压制目标姿势（模型不画你指定的动作）。每姿势先 1 seed 探路，视觉验证通过后再补 seed。**道具姿势同理（2026-08-14 Hypnos 抱枕实测）**：要「抱枕头/拿道具」→ 负向必须删对应的 `holding pillow, hugging pillow, hugging something, carrying object, pillow, cushion, stuffed animal, plushie`（全套都删，缺一都可能让模型不画道具），正向再补 `hugging pillow, holding pillow to chest, arms hugging pillow`。
...
成品：`E:\Hermes workspace\comfyui_workflow\hand_fix_api.json`（模板副本：`templates/hand_fix_api.json`）。
...
**⚠️ 多轮迭代（2026-08-14 实测）**：单轮 hand_fix 常不够——6 指/手指粘连修完可能仍在（qwen-vl 还会误报「5指正常」，以主人肉眼为准）。标准做法：**以每轮输出为下一轮底图循环**（LoadImage 换文件名即可），denoise 阶梯 0.4→0.55→0.65→0.7→0.8 逐轮加码；正向加治 6 指/粘连的强化词 `exactly five fingers on each hand, fingers clearly separated at the base, distinct separate fingers, visible gaps between fingers`；每轮出 2-3 个 denoise/seed 变体让主人肉眼挑，别指望一轮到位。**底图必须健康**（先 PIL std 检查 + 头数检查），坏底图精修会越修越花、颜色越糊。
