...
- Some IDE chat CLIs are GUI-forward rather than terminal agents. Example pattern: TRAE Work CN exposes `trae-cn chat -m ask|edit|agent`, so Orca can launch it or pass a prompt in a terminal, but it may open/reuse the desktop app and not produce a reliable terminal transcript or lifecycle signal. Treat these as launch/prompt handoffs unless the CLI proves it can return terminal output; do not supervise them with orchestration `dispatch --inject` or wait for `worker_done`.
